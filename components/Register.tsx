...
const Register: React.FC<Props> = ({ onRegisterSuccess, onBackToLogin }) => {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Notificar éxito y volver al login
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
        <div className="p-8 bg-emerald-600 text-white text-center">
          ...
          <p className="text-emerald-100 font-medium">Registro de Jugador</p>
        </div>
        ...
        <div className="p-8">
          <form onSubmit={handleRegisterSubmit} noValidate className="space-y-4 max-h-[60vh] overflow-y-auto px-1">
            ...
            >
              {isRegistering ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : 'Registrarme'}
            </button>

            <button type="button" onClick={onBackToLogin} className="w-full text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">
              Volver al Login
            </button>
          </form>
        </div>
